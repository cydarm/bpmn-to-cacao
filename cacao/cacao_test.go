...
<bpmn:definitions xmlns:bpmn="http://www.omg.org/spec/BPMN/20100524/MODEL" xmlns:bpmndi="http://www.omg.org/spec/BPMN/20100524/DI" xmlns:dc="http://www.omg.org/spec/DD/20100524/DC" xmlns:di="http://www.omg.org/spec/DD/20100524/DI" xmlns:bioc="http://bpmn.io/schema/bpmn/biocolor/1.0" xmlns:camunda="http://camunda.org/schema/1.0/bpmn" id="Definitions_0xcsshl" targetNamespace="http://bpmn.io/schema/bpmn" exporter="Camunda Modeler" exporterVersion="3.7.2">
  <bpmn:process id="PrcssAV-EDRAlrt" name="Process AV-EDR Alert" isExecutable="true" camunda:versionTag="Shareable_Workflow">
...
func TestConvertToCacao(t *testing.T) {
	bpmnDefinitions, err := bpmn.ReadBpmn([]byte(inputDataTestString))
	if err != nil {
		t.Fatalf("could not read input: %s", err)
	}
	cacaoPlaybook11, err := cacao.ConvertToCacao(bpmnDefinitions, cacao.CACAO_SPEC_VERSION_11)
	if err != nil {
		t.Fatalf("could not convert BPMN to Cacao: %s", err)
	}
	assert.NotNil(t, cacaoPlaybook11)
	_, err = json.MarshalIndent(cacaoPlaybook11, "", "    ")
	if err != nil {
		t.Fatalf("could not marshal Cacao playbook: %s", err)
	}
	cacaoPlaybook20, err := cacao.ConvertToCacao(bpmnDefinitions, cacao.CACAO_SPEC_VERSION_20)
	if err != nil {
		t.Fatalf("could not convert BPMN to Cacao: %s", err)
	}
	assert.NotNil(t, cacaoPlaybook20)
	_, err = json.MarshalIndent(cacaoPlaybook20, "", "    ")
	if err != nil {
		t.Fatalf("could not marshal Cacao playbook: %s", err)
	}
}
